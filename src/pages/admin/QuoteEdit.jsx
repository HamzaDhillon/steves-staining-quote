...
export default function QuoteEdit() {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      photo_urls: form.photo_urls ? JSON.stringify(form.photo_urls) : null,
    };

    const { data, error } = await supabase.from("quotes").update(payload).eq("id", id);
    ...
    if (!data || data.length === 0) {
      toast.warn("No changes were saved. Please check the form.");
      return;
    }
  ...
  return (
    <div className="container mx-auto px-6 py-10 pt-24 max-w-5xl">
      ...
      {/* Toast container (optional if you have it globally) */}
